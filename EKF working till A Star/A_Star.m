function op_path = A_Star()
    ...
    load(map);
    mdetails
    [MAP, Start, Target, MAX_X, MAX_Y] = create_map(mdetails);
    xStart = Start(1,1)
    yStart = Start(1,2)
    ...
    if ( (xval == xTarget) && (yval == yTarget))
     ...
     op_path = [4*Optimal_path(:,1), 2*Optimal_path(:,2)];
...
function [MAP, Start, Target, MAX_X, MAX_Y]= create_map(mdetails)
    %load mdetails.mat;
    ...
    % Setting Ostacle postion in map
    % Padding in MAP, x to x-1
    for i = 4:size(mdetails,1)
        xval = mdetails(i,1);
        yval = mdetails(i,2);
        MAP(xval,yval) = -1;
        ...
        if( xval-1 > 0)
            MAP(xval-1,yval) = -1;
            plot((xval-1)+.5,yval+.5,'ro');
        end
    end

    % Setting Start Position
    % compliting Map
    xval  = mdetails(2,1);%Starting Position
    yval  = mdetails(2,2);
    Start = [xval yval];
    MAP(xval,yval)=1;
